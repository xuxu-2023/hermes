...
def run_conversation(
    ...
    _scored_memory_context = _ctx.scored_memory_context
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        for idx, msg in enumerate(messages):
            ...
            # Sources: memory manager prefetch + scored relevance context +
            # plugin pre_llm_call hooks with target="user_message" (the default).
            # Both are API-call-time only — the original message in `messages`
            ...
            if idx == current_turn_user_idx and msg.get("role") == "user":
                ...
                if _scored_memory_context:
                    _injections.append(_scored_memory_context)
