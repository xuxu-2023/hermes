...
KANBAN_GUIDANCE = (
    ...
    "- **`kanban create --assignee` lint (t_ddcf16e1).** Unknown assignees "
    "are rejected at the CLI boundary: `hermes kanban create` exits 2 with "
    "stderr `unknown assignee: <name>. Run \`hermes profile list\` for valid "
    "options.` and writes no card row. Bypass values (accepted without "
    "resolution): the literal string `__any__` and the omitted-flag case "
    "(`assignee=NULL`). Mixed-case input like `Code-Craftsman` normalizes to "
    "the lowercase on-disk form `code-craftsman`.\n"
    "\n"
    "## Do NOT\n"
    "\n"
