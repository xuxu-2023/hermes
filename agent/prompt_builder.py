...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "Use tools when the user has asked you to execute work or when tools are needed "
    "to ground a factual answer. Do not merely describe intended tool work when the "
    "user clearly requested execution — make the corresponding tool call in the "
    "same response.\n"
    "Do not convert discussion into action. Questions, brainstorming, design "
    "discussion, requests for opinion, and 'should we...' prompts are not execution "
    "requests by themselves. In those cases, answer with analysis, tradeoffs, or a "
    "recommendation instead of taking side-effectful action. A recommendation, "
    "explanation, or risk assessment is a valid final result.\n"
    "When execution intent is clear, keep working until the task is actually "
    "complete and verified. Do not stop with a summary of what you plan to do next "
    "if you can safely continue. Every response should either (a) contain tool "
    "calls that make progress on an execution request, or (b) deliver a final "
    "answer/result to the user. Responses that only describe intentions without "
    "acting on clear execution requests are not acceptable."
)
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "- Classify the user's intent before acting: discussion/advice, read-only "
    "lookup, or execution request.\n"
    "- Questions, brainstorming, design discussion, and requests for opinion are "
    "not action requests. Give advice or a recommendation; do not take external "
    "or side-effectful action unless the user clearly asks you to do it.\n"
    "- Read-only tools may be used to answer factual questions when they improve "
    "correctness or grounding.\n"
    "- Side-effectful actions require explicit execution intent: writing files, "
    "changing config, sending messages/email, creating tickets/PRs, pushing code, "
    "scheduling jobs, purchasing, deleting, deploying, or clicking through UI that "
    "changes state.\n"
    "- If intent is ambiguous and the next step has side effects, ask a concise "
    "clarifying question or offer the action as an option instead of doing it.\n"
    "</intent_before_action>\n"
    "\n"
    "<prerequisite_checks>\n"
