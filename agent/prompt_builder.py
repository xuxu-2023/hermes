...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Work in the workspace.** `cd $HERMES_KANBAN_WORKSPACE` before file "
    "operations. Don't modify files outside it unless asked. If `read_file` "
    "reports a truncated default read, do not write or commit from that page; "
    "re-read with explicit `offset`/`limit` or block.\n"
    "3. **Heartbeat on long operations.** Call `kanban_heartbeat(note=...)` "
    "every few minutes during long subprocesses. Skip short tasks. "
    "**If your task may run longer than 1 hour, call `kanban_heartbeat` "
    "at least hourly** — the dispatcher "
