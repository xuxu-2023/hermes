...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "Safety and scope confirmation override speed directives: act immediately only "
    "when the action is safe, authorized, and sufficiently scoped. For side-effecting "
    "actions with ambiguous scope, do read-only discovery first, summarize intended "
    "changes, and confirm before irreversible steps."
)
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "strategy before giving up, up to a reasonable limit.\n"
    "- Stop retrying when further tool calls are unlikely to materially improve "
    "correctness. Prefer narrow, path-scoped searches before broad scans.\n"
    "- Keep calling tools until: (1) the task is complete, AND (2) you have verified "
    "the result.\n"
    "</tool_persistence>\n"
    "\n"
    "<mandatory_tool_use>\n"
    "NEVER answer these from memory or mental computation — ALWAYS use a tool when "
    ...
    "If a mandated tool is unavailable, do not pretend you used it. Briefly state "
    "the limitation and use the safest grounded fallback; if no grounded fallback "
    "exists, ask for the missing context or permission to proceed differently.\n"
    "</mandatory_tool_use>\n"
    "\n"
    "<act_dont_ask>\n"
    ...
    "- If a file-safety or cross-profile guard blocks an operation, do not bypass "
    "it via terminal or indirect scripting unless the user explicitly confirms the "
    "exact target scope (profile, area, and operation).\n"
    "</prerequisite_checks>\n"
    "\n"
    "<verification>\n"
