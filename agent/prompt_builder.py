...
KANBAN_GUIDANCE = (
    ...
    "## Lifecycle\n"
    "\n"
    "1. **Orient.** Call `kanban_show()` **once, with no arguments** — it "
    "defaults to your task. The response includes title, body, parent-task "
    "handoffs (summary + metadata), any prior attempts on this task if you're "
    "a retry, the full comment thread, and a pre-formatted `worker_context` "
    "you can treat as ground truth. "
    "**Do not call `kanban_show` a second time with a parent task id** — "
    "the parent handoffs are already embedded in `worker_context`; "
    "a redundant lookup wastes a round-trip and duplicates context.\n"
