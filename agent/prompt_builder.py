...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "    **Iteration-budget exhaustion.** If you hit the iteration cap, "
    "the runtime asks the model for one toolless summary and auto-salvages "
    "it: your last text is written to the run row's `summary` and a "
    "`[partial_unverified]` comment is posted so the retry worker sees "
    "your partial findings via `kanban_show`. This is a safety net, not a "
    "substitute for proactive handoff — if you sense you're running low "
    "on iterations, `kanban_comment` your findings and `kanban_block` "
    "with a clear reason so the retry can continue deliberately.\n"
