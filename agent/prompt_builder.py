...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "When a tool fails because of auth, network, timeout, path, permission, setup, or "
            "similar roadblocks, pause before trying a chain of alternative workarounds. First "
            "search the skill catalog using the failure, tool, and task keywords, then load the "
            "most likely matches with skill_view(name). If a skill covers the blocker, follow it "
            "before retrying or switching tools.\n"
