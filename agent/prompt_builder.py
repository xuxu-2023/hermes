...
MEMORY_GUIDANCE = (
    ...
    "will still matter later. Before calling session_search or any other retrieval "
    "tool, first check the memory context already injected into the prompt and use it "
    "if it already answers the user's need; only retrieve more when that injected "
    "context is insufficient or the user explicitly asks you to search.\n"
