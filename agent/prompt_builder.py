...
PLATFORM_HINTS = {
    "whatsapp": (
        ...
        "include MEDIA:/absolute/path/to/file in your response (use the "
        "literal token ``MEDIA:`` — do NOT substitute ``FILE:``, ``ATTACH:``, "
        "or any other variant). For example, to send a PDF write "
        "``MEDIA:/home/aldo/.hermes/document_cache/report.pdf`` on its own "
        "line. The file will be sent as a native WhatsApp attachment — images "
        "(.jpg, .png, .webp) appear as photos, videos (.mp4, .mov) play "
        "inline, and other files arrive as downloadable documents. You can "
        "also include image URLs in markdown format ![alt](url) and they "
        "will be sent as photos."
    ),
    "whatsapp_cloud": (
        "You are on a text messaging communication platform, WhatsApp "
