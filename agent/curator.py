...
CURATOR_REVIEW_PROMPT = (
    ...
    "enough. Before creating, run the PRE-SAVE GATE below. If a current "
    "skill is close enough, patch the existing skill instead of creating "
    "a new one. Only use skill_manage action=create for a genuinely new "
    "class-level workflow. Its SKILL.md must cover the shared workflow, "
    "include short labeled subsections, and include `do_not_use_when:` "
    "negative examples in frontmatter plus a body section named `When "
    "NOT to use`. Archive the now-absorbed narrow siblings.\n"
    ...
    "merges.\n\n"
    "PRE-SAVE GATE for every skill_manage action=create — not optional:\n"
    "1. Trigger: define the exact future task pattern that should load "
    "this skill. If it is one incident, do not create it.\n"
    "2. Reusability: confirm this workflow is likely to recur or applies "
    "to a class of tasks. Otherwise move it to references/ under an "
    "existing skill or leave it out.\n"
    "3. Artifact: identify the durable artifact being saved — commands, "
    "template, checklist, API quirk, or verified procedure.\n"
    "4. Guardrail: include at least one `do_not_use_when:` frontmatter "
    "negative example and a `When NOT to use` body section so future "
    "agents do not overgeneralize it.\n"
    "5. Verification: state how a future agent verifies the procedure still "
    "works.\n"
    "Dedup-check: before creating, search the inventory with skills_list "
    "and skill_view for the top overlapping candidates. If any existing "
    "skill can be broadened, patch the existing skill instead.\n\n"
    "Your toolset:\n"
