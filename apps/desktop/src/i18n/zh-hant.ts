...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      remoteUrlDesc: '使用者看到的遠端閘道公開 URL。支援路徑前綴，例如 /hermes。',
      transportTitle: '傳輸方式',
      transportDesc: '選擇 Desktop 直接連線閘道，或經由本機 mTLS proxy 連線。',
      transportDirect: '直接連線',
      transportLocalMtlsProxy: '本機 mTLS proxy',
      effectiveUrlTitle: '本機 proxy URL',
      effectiveUrlDesc: 'Desktop 實際使用的進階傳輸端點；公開閘道 URL 仍會顯示在設定中。',
      ...
      connectedToViaProxy: (publicUrl, effectiveUrl, version) =>
        `已連線至 ${publicUrl}，經由 ${effectiveUrl}${version ? ` · Hermes ${version}` : ''}`,
