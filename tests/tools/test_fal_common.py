...
class TestImportFalClient:
    def test_returns_fal_client_module(self):
        """import_fal_client returns the fal_client module reference."""
        result = import_fal_client()
        assert result is not None
        assert hasattr(result, "__name__")
        assert result.__name__ == "fal_client"

    def test_lazy_ensure_import_error_is_swallowed(self):
        """If lazy_deps.ensure raises ImportError, it's swallowed (fal_client still imported)."""
        with patch("tools.lazy_deps.ensure", side_effect=ImportError("no lazy_deps")):
            result = import_fal_client()
            assert result is not None
            assert result.__name__ == "fal_client"
    ...
    def test_lazy_ensure_module_missing_is_swallowed(self):
