...
# ------------------------------------------------------------------
# Self-echo filtering (sent-msg tracking) tests
# ------------------------------------------------------------------
...
class TestSelfEchoFiltering:
    ...
    def test_sent_msg_echo_is_dropped(self):
        """Test that inbound messages matching sent msg_ids are dropped."""
        # Arrange
        adapter = self._make_adapter()
        adapter._sent_msg_ids = collections.OrderedDict()

        # Simulate sending a message
        test_msg_id = "ROBOT1.0_test123"
        adapter._track_sent_msg(test_msg_id)

        # Act - simulate echo
        event_d = {
            "id": test_msg_id,
            "content": "⏳ Working — 5 min",
            "author": {"user_openid": "test_user"},
            "timestamp": "1234567890",
        }

        # Mock _is_duplicate to return False (new message)
        with patch.object(adapter, "_is_duplicate", return_value=False):
            with patch.object(adapter, "_handle_c2c_message") as mock_handle:
                asyncio.run(adapter._on_message("C2C_MESSAGE_CREATE", event_d))

                # Assert - should NOT call _handle_c2c_message
                mock_handle.assert_not_called()

                # Assert - msg_id should be removed from tracking
                assert test_msg_id not in adapter._sent_msg_ids

    def test_normal_inbound_is_not_dropped(self):
        """Test that normal user messages are not affected."""
        # Arrange
        adapter = self._make_adapter()
        adapter._sent_msg_ids = collections.OrderedDict()

        # Simulate a normal user message (different ID)
        user_msg_id = "ROBOT1.0_user456"
        ...
        with patch.object(adapter, "_is_duplicate", return_value=False):
            with patch.object(adapter, "_handle_c2c_message") as mock_handle:
                ...
                # Assert - should call _handle_c2c_message
                mock_handle.assert_called_once()

    def test_track_sent_msg_evicts_oldest(self):
        ...
        assert len(adapter._sent_msg_ids) == 500
        assert "msg_0" not in adapter._sent_msg_ids
        assert "msg_1" not in adapter._sent_msg_ids
        assert "msg_501" in adapter._sent_msg_ids
