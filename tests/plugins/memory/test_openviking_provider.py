...
def test_openviking_legacy_env_names_are_runtime_fallbacks(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setenv("OPENVIKING_ENDPOINT", "http://legacy.local")
    monkeypatch.setenv("OPENVIKING_AGENT", "legacy-agent")
    ...
    assert settings["url"] == "http://legacy.local"
...
def test_openviking_desktop_save_rejects_missing_profile_path(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    hermes_home = tmp_path / "hermes"
    hermes_home.mkdir()
    missing_path = tmp_path / "ovcli.conf.missing"
