...
Step 1 — tool call:
```json
{ "group_code": "<group_code>", "action": "find", "name": "元宝", "mention": true }
```
...
```json
yb_query_group_info({ "group_code": "<group_code>" })
```
...
- `group_code` comes from chat_id: `group:<group_code>` → `<group_code>`
