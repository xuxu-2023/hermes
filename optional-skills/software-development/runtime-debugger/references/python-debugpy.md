# Python Debugger (pdb + debugpy) Reference

Full content from the original `python-debugpy` skill.
...
5. **Attach to PID fails on hardened kernels.** Use `echo 0 > /proc/sys/kernel/yama/ptrace_scope`.
6. **Threads.** `pdb` only debugs the current thread. For multithreaded code, use `debugpy`.
7. **`asyncio.** `pdb` works in coroutines but `await` inside pdb requires Python 3.13+ or `interact` mode.
