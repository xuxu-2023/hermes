...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _is_interactive_operator_authorized(self, open_id: str, user_id: str = "") -> bool:
        ...
        Interactive card callbacks already come from a chat where Hermes sent a
        gated prompt. When an explicit Feishu allowlist/admin list exists we
        still enforce it, but without one we should not reject the same user who
        could already approve via plain-text ``/approve``.
        """
        sender_ids = {
        ...
        if not allowed_ids:
            return True
        return "*" in allowed_ids or bool(sender_ids & allowed_ids)
    ...
    async def _resolve_update_prompt(
        ...
        *,
        ...
        if not state:
            logger.debug("[Feishu] Update prompt %s already resolved or unknown", prompt_id)
            return
        if open_id and not self._is_interactive_operator_authorized(open_id, user_id):
