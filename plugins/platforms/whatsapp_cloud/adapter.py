...
def _is_connected(config) -> bool:
    """WhatsApp Cloud is considered connected when both required Graph API
    credentials are set.

    Looks up via ``hermes_cli.gateway.get_env_value`` (not ``os.getenv``) so
    setup-status callers that patch ``get_env_value`` observe the same value.
    Matches the discord/whatsapp plugin pattern.
    """
    import hermes_cli.gateway as gateway_mod
    phone_id = (gateway_mod.get_env_value("WHATSAPP_CLOUD_PHONE_NUMBER_ID") or "").strip()
    access_token = (gateway_mod.get_env_value("WHATSAPP_CLOUD_ACCESS_TOKEN") or "").strip()
    return bool(phone_id and access_token)


def _build_adapter(config):
    """Factory wrapper that constructs WhatsAppCloudAdapter from a PlatformConfig."""
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        platform_hint=(
            "WhatsApp Cloud API — supports text, images, documents, audio, "
            "video, and interactive buttons. Messages support markdown-style "
            "bold (*text*) and italic (_text_). Max message length is 4096 "
            "characters."
        ),
    )
