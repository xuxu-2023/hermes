...
_REASON = "casual/no-action Telegram group message"
...
_APPROVED_NO_ACTION_TEST_RE = re.compile(
    r"^\s*(?:fgd-123\s*)?test\s*1\s*(?:[-:–—]\s*casual/no-action(?:\s+message)?)?\s*$",
    re.IGNORECASE,
)

_ZERO_OUTPUT_QA_RE = re.compile(
...
def _is_telegram_group_context(source: Any) -> bool:
    ...
    return chat_type in {"group", "supergroup", "channel", "thread", "forum"}


def _is_approved_no_action_test_message(text: str) -> bool:
    """Return True for the bounded FGD-123 Test 1 suppression probe."""
    return bool(_APPROVED_NO_ACTION_TEST_RE.match(text or "")) or bool(
        _ZERO_OUTPUT_QA_RE.match(text or "")
    )
...
def _is_obvious_casual_no_action(text: str) -> bool:
    """Return True only for low-signal group chatter.

    The pre-dispatch gate should be conservative: preserve anything that
    looks like a request, routing packet, dev/approval work, or direct Jimmy
    address, and only skip plain group chatter that has no preserve signal.
    """
    stripped = (text or "").strip()
    if not stripped:
        return False

    if _is_approved_no_action_test_message(stripped):
        return True

    if _has_preserve_signal(stripped):
        return False
...
def _on_pre_gateway_dispatch(event: Any, **_: Any) -> Optional[dict[str, str]]:
    ...
    if not _is_obvious_casual_no_action(text):
        return None
