    ...
    def _get_client(self):
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                # HindsightEmbedded only accepts LLM/idle settings in its
                # constructor, but ensure_running() later passes self.config
                # to hindsight-embed. Include the full generated daemon env so
                # embedding-provider overrides survive even if the profile env
                # file was stale or was regenerated by an older process.
                if hasattr(self._client, "config"):
                    self._client.config.update(_build_embedded_profile_env(dict(self._config or {})))
            else:
