...
| subagent 达到 max_iterations 但代码未 push | subagent 完成代码修改但卡在 git push 阶段，超时后停在"未提交"状态 | **subagent 交付后立即执行 `git status` 检查，如果有 modified/untracked 文件说明代码已改但未 commit。正确流程：1) `git stash` 先暂存 2) `git checkout master` 切到 master 确认 build 环境正常 3) `git checkout v20-emotion-tracking && git stash pop` 切回功能分支 4) `git add` + `git commit` + `git push`。多次实践发现：subagent 通常能在 max_iterations 内完成代码修改，但几乎无法完成 git add/commit。建议：代码修改完成后主 agent 直接接手 stash→checkout master 验证 build→切回 commit/push** |
| subagent 交付完成但代码在 workspace 而非克隆目录 | subagent 正常工作完毕，但代码写入 `~/.hermes/proposals/workspace-dev/proposals/<project>/` 而非临时克隆目录。需要找到实际文件位置才能 commit/push | **subagent 交付后：1) 先 `find /home/hermes -name \"db.ts\" -o -name \"trading.ts\"` 搜索关键输出文件定位实际工作目录 2) 在该目录执行 `git status` 检查 3) 确认是所需项目后 git add/commit/push。如果项目已存在 git clone，直接在该克隆目录检查 `git log` 确认分支历史** |
