...
#### 发现 1: http.client 直接调用失败，用 curl subprocess 才行

Python `http.client.HTTPSConnection` 在 WSL 中调用 GitHub API 会返回 `403 "Not Found"` 或 `{"message": "Bad credentials"}`。改用 `subprocess.run(['curl', ...])` 替代 `http.client` 后完全正常：
...
#### 发现 2: "Reference already exists" → 用 PATCH + force=true

第一次 `POST /git/refs` 创建分支即可。如果分支已存在（之前创建过但 PR merge 后 ref 还在），再次 POST 会报 "Reference already exists"。解决：PATCH 加 `force: true`：

```python
payload = json.dumps({'sha': new_commit_sha, 'force': True})
...
#### 发现 3: 获取分支 SHA 必须用完整40字符

```bash
# 获取 master 当前 SHA（完整40字符）
gh api repos/{owner}/{repo}/git/refs/heads/master --jq '.object.sha'
# → 7b5d7062579c4566e308cb4ba142ffb7902e76eb

# 用这个 SHA 获取 tree
gh api repos/{owner}/{repo}/git/commits/7b5d706... --jq '.tree.sha'
```

#### 发现 4: tree entries 的 path 精确匹配

构建 new_entries 时 path 必须与 blob_map key 完全匹配，建议用精确路径映射。
```bash
