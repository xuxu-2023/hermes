...
## ⚠️ SSL EOF 错误 — 用 curl 代替 Python http.client

**问题**：Python `http.client.HTTPSConnection` 连接 api.github.com 时会报：
```
ssl.SSLEOFError: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol
```
即使设置 `ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)` 也不行。

**解决**：用 `curl` via `subprocess.run()` 代替 Python http.client。
...
result = subprocess.run(
    ['curl', '-s', '-X', 'POST',
     ...
     '-d', '{"content": ' + json.dumps(content) + ', "encoding": "utf-8"}',
...
**错误**：直接 `curl` 获取 commit 返回的 SHA 可能不对。
```bash
# ❌ 可能返回 truncated 或不同的数据
curl -s "https://api.github.com/repos/$OWNER/$REPO/git/commits/$SHA" | python3 -c "import sys,json; print(json.load(sys.stdin)['tree']['sha'])"
```

**正确**：用 `gh api --jq` 获取精确的 40 字符 SHA。
```bash
# ✅ 可靠
TREE_SHA=$(gh api repos/$OWNER/$REPO/git/commits/$PARENT --jq '.tree.sha')
# 或
...
## ⚠️ 分支保护 — deploy.yml 只在 master 运行

如果 deploy.yml 设置为只在 master 分支运行，feature 分支的 CI 不会触发。

**解决**：创建 PR 并 squash merge 到 master，GitHub Actions 会自动在 master 上运行。

```bash
# 1. 创建 PR
gh api repos/$OWNER/$REPO/pulls -X POST \
  -f title="feat: ..." -f head="$BRANCH" -f base="master"

# 2. 合并 PR（会触发 master 上的 workflow）
gh api repos/$OWNER/$REPO/pulls/$PR_NUM/merge -X PUT \
  -f merge_method=squash -f commit_title="feat: ... (squashed)"
```

## 完整 curl + subprocess 推送脚本模板

```python
import json, subprocess

GH_TOKEN = subprocess.run(['gh', 'auth', 'token'], capture_output=True, text=True).stdout.strip()
OWNER, REPO = 'YeLuo45', 'pixel-pal-web'
PARENT = '<parent-commit-sha>'      # 40字符完整SHA
TREE_SHA = '<parent-tree-sha>'       # 从 parent commit 的 tree 字段获取
BASE_DIR = '/path/to/project'

# Step 1-2: 读取文件，创建 blobs
files = ['src/a.ts', 'src/b.tsx']
blob_map = {}
for filepath in files:
    content = open(f'{BASE_DIR}/{filepath}').read()
    result = subprocess.run(
        ['curl', '-s', '-X', 'POST',
         '-H', f'Authorization: Bearer {GH_TOKEN}',
         '-H', 'Content-Type: application/json',
...
# Step 3: 获取现有 tree entries（从 parent tree SHA）
tree_data = json.loads(subprocess.run(
    ['curl', '-s', '-H', f'Authorization: Bearer {GH_TOKEN}',
     f'https://api.github.com/repos/{OWNER}/{REPO}/git/trees/{TREE_SHA}?recursive=1'],
    capture_output=True, text=True, timeout=30
).stdout)

# Step 4: 构建新 tree（替换变更文件，保留其他）
new_entries = []
for item in tree_data['tree']:
    if item['path'] in blob_map:
        new_entries.append({**item, 'sha': blob_map[item['path']]})
    else:
        new_entries.append(item)
for p, sha in blob_map.items():
    if not any(e['path'] == p for e in new_entries):
        new_entries.append({'path': p, 'sha': sha, 'mode': '100644', 'type': 'blob'})

# Step 5: 创建新 tree
new_tree_resp = json.loads(subprocess.run(
    ['curl', '-s', '-X', 'POST',
     '-H', f'Authorization: Bearer {GH_TOKEN}',
     '-H', 'Content-Type: application/json',
     '-d', json.dumps({'tree': new_entries, 'base_tree': TREE_SHA}),
...
# Step 6: 创建 commit
commit_resp = json.loads(subprocess.run(
    ['curl', '-s', '-X', 'POST',
     '-H', f'Authorization: Bearer {GH_TOKEN}',
     '-H', 'Content-Type: application/json',
     '-d', json.dumps({'message': 'feat: ...', 'tree': new_tree_sha, 'parents': [PARENT]}),
...
subprocess.run(
    ['curl', '-s', '-X', 'POST',
     '-H', f'Authorization: Bearer {GH_TOKEN}',
     '-H', 'Content-Type: application/json',
     '-d', json.dumps({'ref': f'refs/heads/{BRANCH}', 'sha': new_commit_sha}),
     f'https://api.github.com/repos/{OWNER}/{REPO}/git/refs'],
    capture_output=True, text=True, timeout=30
)
```

**注意**：所有 `subprocess.run` 的 `-s` 参数去掉可以看详细输出用于调试。
