...
| H | 新植物 Zapricot/Cattail/Gloom Shroom | 8332563 |
| I | 成就系统 UI + 统计面板 + StatsManager | a732f90 |
...
for z in self.zombies:
    if z.poisoned:
        z.poison_timer += dt
        if z.poison_timer >= 0.5:
            z.poison_timer = 0
            z.hp -= 10  # 每0.5秒扣10血，共4次=2秒40伤害
            if z.hp <= 0:
                z.dead = True
```

### 4. Paged Card Panel 模式
...
| 电弧闪电 | ElectricArcEffect | source/component/bullet.py |
| 毒雾孢子 | GloomShroomSporeExplosion | source/component/effects.py |
| 报纸撕裂 | NewspaperShredEffect | source/component/effects.py |
