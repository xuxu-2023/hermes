...
def _sanitize_node(node: Any, path: str) -> Any:
    ...
    for key, value in node.items():
        ...
        if key in {"properties", "$defs", "definitions"} and isinstance(value, dict):
            ...
            #  - ``default`` / ``const``: a single literal instance value
            #    (any JSON type, including arrays and objects)
            # Recursing into these with _sanitize_node() would mis-interpret
            # literal strings like "path" as bare-string schemas and replace
            # them with {"type": "object"} dicts — corrupting list/object
            # ``default`` values such as ``["read", "write"]``. Pass through
            # unchanged.
