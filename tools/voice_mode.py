...
def play_beep(frequency: int = 880, duration: float = 0.12, count: int = 1) -> None:
    """Play a short beep tone via system audio player (afplay/ffplay/aplay).
    ...
    try:
        ...
        # Write to a temp WAV and play via system player to avoid sounddevice
        # playback paths (CoreAudio CFFI race causes EXC_BAD_ACCESS at 0x4).
        tmp_path = None
        try:
...
def play_audio_file(file_path: str) -> bool:
    ...
    # Use system audio players (Popen for interruptability via stop_playback)
