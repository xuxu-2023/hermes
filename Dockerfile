...
#
# `uv sync --frozen --no-install-project --extra all --extra messaging --extra feishu`
# installs the deps reachable through the composite `[all]` extra
# (handpicked set intended for the production image — excludes `[dev]`),
# plus gateway messaging adapters that should work in the published image
# without a first-boot lazy install.  Feishu ([feishu]) is included because
# lazy installs are disabled in the container (HERMES_DISABLE_LAZY_INSTALLS=1
# and a read-only venv), so it must be baked into the image like the other
# messaging platforms.  We do NOT use `--all-extras`:
...
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all --extra messaging --extra anthropic --extra bedrock --extra azure-identity --extra feishu --extra hindsight --extra matrix
