...
def check_for_updates() -> Optional[int]:
    ...
    # installs: `check_via_pypi()` compares against VERSION, so a `pip install
    # --upgrade` changes VERSION but leaves rev unchanged (both None) (#34491).
    # The HEAD guard matters for source installs updated by hand: `git pull`
    # moves HEAD but leaves VERSION and rev unchanged, so without it the stale
    # "behind" count would survive the 6h TTL (#40944).
    now = time.time()
    try:
