...
async def _https_redirect_middleware(request: Request, call_next):
    """Redirect plain-HTTP requests to HTTPS when behind a TLS-terminating proxy.

    When the auth gate is active (non-loopback bind) the dashboard is
    expected to be behind a TLS-terminating reverse proxy (Tailscale
    Serve, Fly.io, Cloudflare Tunnel, etc.).  If a client connects via
    plain HTTP — e.g. the Electron desktop app loading
    ``http://host:port/login`` instead of the proxy's HTTPS URL — the
    PKCE cookie is set with ``SameSite=Lax`` and no ``Secure`` flag,
    and on a *different hostname* than the OAuth callback will use.

    The OAuth redirect_uri is always the HTTPS public URL, so the
    callback comes back on a different host than where the cookie was
    set.  Cookies are host-scoped, so the callback never sees the
    PKCE cookie and the login fails with "Missing PKCE state cookie".

    Redirecting the client to HTTPS *before* any cookie is set ensures
    the entire OAuth round-trip stays on one hostname with ``Secure``
    cookies.

    The redirect target is built from ``HERMES_DASHBOARD_PUBLIC_URL``
    (or ``dashboard.public_url``) when set; otherwise from
    ``X-Forwarded-Host`` + ``X-Forwarded-Proto``.  If neither is
    available the request passes through (we can't construct a
    meaningful HTTPS target).
    """
    if not getattr(app.state, "auth_required", False):
        return await call_next(request)
    # Already HTTPS (direct or via proxy_headers rewrite) — no redirect needed.
    if request.url.scheme == "https":
        return await call_next(request)
    # Only redirect GET/HEAD — other methods may carry bodies that
    # can't be re-sent after a redirect, and non-browser clients
    # (curl, the session-token probe) should get the raw response.
    ...
    if public_url:
        # Operator declared a public URL — use it as the HTTPS target.
        from urllib.parse import urlsplit, urlunsplit

        parts = urlsplit(str(request.url))
        pub = urlsplit(public_url)
        target = urlunsplit(
            parts._replace(scheme="https", netloc=pub.netloc)
        )
    elif request.headers.get("x-forwarded-host"):
        # No public_url but we have X-Forwarded-Host from the proxy.
        # Construct the HTTPS URL from it (strip any port the proxy
        # fronted, since TLS is on 443).
        from urllib.parse import urlsplit, urlunsplit

        parts = urlsplit(str(request.url))
        fwd_host = request.headers["x-forwarded-host"].split(",")[0].strip()
        target = urlunsplit(
            parts._replace(scheme="https", netloc=fwd_host)
        )
    else:
        # No way to know the public HTTPS hostname — pass through.
        return await call_next(request)
