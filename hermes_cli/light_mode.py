"""Light/dark terminal detection + skin-color remapping for the CLI.

Extracted verbatim from ``cli.py`` (which had grown past 14k lines).  This
module owns the pure logic: hex→ANSI conversion, terminal background
detection (env hints, COLORFGBG, OSC 11 probe), and the dark→light color
remap table used to keep skin colors readable on light backgrounds.

Import-time side effects intentionally live in ``cli.py``, not here:

  * ``_install_skin_light_mode_hook()`` wraps ``SkinConfig.get_color`` and
  * the OSC 11 prime warms ``_detect_light_mode()`` before prompt_toolkit
    grabs the tty.

Keeping those in ``cli.py`` means ``import hermes_cli.light_mode`` stays a
pure import — tests and subagents can pull in the detection helpers without
triggering a terminal query or monkeypatching the skin engine.
"""
...
# ANSI building block shared with cli.py's conversation display.
