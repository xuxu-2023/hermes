/**
 * Module-level subagent state for the web dashboard.
 *
 * The desktop app uses nanostores atoms; the web dashboard keeps a simple
 * subscription-based store compatible with useSyncExternalStore so pages
 * can react to live subagent events without pulling in a state library.
 *
 * Data arrives via gateway WebSocket events (subagent.spawned,
 * subagent.progress, subagent.completed, subagent.failed, etc.).
 * Callers wire those events into upsertSubagent() / clearSessionSubagents().
 */
...
const idOf = (p: SubagentPayload) =>
  str(p.subagent_id) ||
  `${str(p.parent_id) || "root"}:${num(p.task_index) ?? 0}:${str(p.goal)}`;
...
function streamFromPayload(
  ...
  for (const tail of asTail(payload.output_tail)) {
    const line = tail.tool
      ? formatTool(tail.tool, tail.preview ?? "")
      : compact(tail.preview ?? "");
    if (line) {
      out.push({
        at,
        isError: tail.isError,
        kind: tail.tool ? "tool" : "progress",
        text: line,
      });
    }
  }
...
function toProgress(
  ...
  const tool = str(payload.tool_name);
  const stream = streamFromPayload(payload, status, eventType, at).reduce(
    appendStream,
    prev?.stream ?? [],
  );
...
type SubagentMap = Record<string, SubagentProgress[]>;
type Listener = () => void;

let state: SubagentMap = {};
...
export function getSubagentsBySession(): SubagentMap {
  return state;
}
...
export function buildSubagentTree(
  ...
  const walk = (node: SubagentNode) =>
    node.children.sort(sort).forEach(walk);
