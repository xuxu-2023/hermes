...
`documents` 选项会把文件安装到 `stateDir` 下的显式路径。每个键都会被当作相对于 `${services.hermes-agent.stateDir}`（默认 `/var/lib/hermes`）的目标路径。

- 对 Hermes 从 `$HERMES_HOME` 读取的文件，使用 `.hermes/...`
- 对属于 `workingDirectory` 的项目上下文文件，使用 `workspace/...`
- 其他键会按你写出的路径原样放到 `stateDir` 下
...
值可以是内联字符串或路径引用。嵌套目录会自动创建，文件会在每次 `nixos-rebuild switch` 时安装。
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `documents` | `attrsOf (either str path)` | `{}` | 安装到 `stateDir` 下的文档。键为相对于 `stateDir` 的目标路径；激活时会自动创建嵌套父目录 |
