...
```
computer_use(action="capture", mode="som", app="Helium")
```
...
wait              seconds=0.5
list_apps
focus_app         app="Helium"  raise_window=false   (default: don't raise)
```
...
2. **将截图范围限定到某个应用**（`app="Helium"`）——噪音更少，元素更少，不会泄露用户打开的其他窗口。在 Kamell 这台机器上，浏览器登录/GUI 工作优先使用 Helium，只有 Helium 不可用时才用 Chrome，不用 Arc 或 Safari。
