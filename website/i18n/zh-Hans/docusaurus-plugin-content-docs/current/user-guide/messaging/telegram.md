...
Telegram 机器人有一个**隐私模式**，**默认启用**。这是在群组中最容易踩坑的地方。
...
**使用 `auto` 或 `draft` 时私聊中的效果** — Telegram 会显示按 token 逐步更新的动态草稿预览。回复完成后，它作为普通消息投递，草稿预览在客户端自然清除。草稿没有消息 ID，因此最终答案才是保留在聊天历史中的内容。
