...
Curator **绝不触碰**通过 [agentskills.io](https://agentskills.io) 安装的 hub 技能。它只审查 agent 自身创作的技能。默认情况下，随仓库附带的捆绑技能在长期未使用后也可被修剪——详见下方 [`prune_builtins`](#配置) 配置。它也**绝不自动删除**——最坏的结果是归档到 `~/.hermes/skills/.archive/`，这是可恢复的。
...
curator:
  ...
  prune_builtins: true         # 捆绑技能也可被 curator 审查
```

若要完全禁用，设置 `curator.enabled: false`。

:::tip 捆绑技能修剪
默认情况下（`prune_builtins: true`），随仓库附带的捆绑技能**同样**可被 curator 审查——它们会累积使用遥测数据，不活动时钟从 curator 首次看到它们时开始计时。长期未使用的捆绑技能仅在真正未使用 `archive_after_days` 天后才会被归档，绝不会在首次运行时被批量修剪。设置 `prune_builtins: false` 可永久保留所有捆绑内置技能。Hub 安装的技能**无论此标志如何设置都不会被修剪**。
:::
