...
由 [Nous Research](https://nousresearch.com) 构建的自我改进 AI 智能体。唯一一个内置学习循环的智能体——它从经验中创建技能，在使用过程中持续改进，主动提醒自己持久化知识，并在会话之间不断加深对你的理解。
...
请参阅完整的 **[安装指南](/getting-started/installation)**，了解安装程序的具体操作、普通用户安装与 root 安装的目录布局以及 Windows 相关说明。

## Hermes Agent 是什么？

它不是只能待在 IDE 里的 AI 编程助手，也不是某个 API 外面套一层壳的聊天机器人。Hermes 是一个**自主智能体**：你用得越久，它越了解你的工作流，也越会帮你做事。它可以跑在任何地方：5 美元的 VPS、GPU 集群，或者闲置时几乎不花钱的 serverless 基础设施（Daytona、Modal）。你可以在 Telegram 上和它聊天，同时让它在一台你从没 SSH 登录过的云端虚拟机里干活。它不需要一直占着你的本地电脑。
...
- **在你常用的平台上** — CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost、Email、SMS、DingTalk、Feishu、WeCom、Weixin、QQ Bot、元宝、BlueBubbles、Home Assistant、Microsoft Teams、Google Chat 等——通过一个网关支持 20+ 平台
- **由模型训练者构建** — 由 [Nous Research](https://nousresearch.com) 创建，该实验室是 Hermes、Nomos 和 Psyche 背后的团队。支持 [Nous Portal](https://portal.nousresearch.com)、[OpenRouter](https://openrouter.ai)、OpenAI 或任意兼容端点
