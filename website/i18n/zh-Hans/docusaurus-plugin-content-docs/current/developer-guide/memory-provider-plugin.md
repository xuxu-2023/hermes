...
:::info 独立分发
新的 memory provider 应发布为独立插件仓库，而不是在 `plugins/memory/` 下新增目录。用户可以将其安装到 `$HERMES_HOME/plugins/<name>/`，或通过包安装器 / entry point 安装；已有内置提供商仍使用仓库内布局。
:::

## 目录结构

对于独立 provider，请在解包后的插件目录中提供同样的文件结构，例如 `$HERMES_HOME/plugins/my-provider/`：
...
已有内置提供商在 Hermes 仓库内的 `plugins/memory/<name>/` 下使用同样的文件结构。
