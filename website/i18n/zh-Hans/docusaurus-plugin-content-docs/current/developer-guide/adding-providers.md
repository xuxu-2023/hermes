...
当你添加一个插件并调用 `register_provider()` 时，以下内容会自动串联起来：
...
## 第 5 步：在 `hermes_cli/main.py` 中集成 CLI
...
开发者可以把 provider 集成得很完善，但仍然让用户无法发现所需的环境变量或 setup 流程。
...
- [ ] 在 `hermes_cli/main.py` 中添加 CLI 集成（setup.py 自动继承）
