...
| `commented_through_completion` | `{pid, claimer, exit_code, comment_id, comment_bytes}` | Worker exited successfully while the task was still `running` BUT had posted a non-trivial handoff comment (≥ 500 bytes referencing a durable artifact like a `data/` path, `docs/` path, `/tmp/` handoff file, `.md`/`.json`/`.txt` filename, HTTP URL, or PR reference) during the current run. The dispatcher treats this as a successful run — promotes the task to `done`, ends the run with `outcome=completed`, and does NOT trip the circuit breaker. This is the "research/audit worker ships the report as a comment then exits rc=0" pattern; without this carve-out, such runs were misclassified as protocol violations and duplicate-respawned. |
