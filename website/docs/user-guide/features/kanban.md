...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a GitHub PR at or after the latest explicit unblock (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up, while still allowing PR-feedback bridge events to wake a blocked PR task when only older PR handoff comments contain raw PR URLs. See the `respawn_guarded` row in the [event reference](#event-reference).
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (a GitHub PR URL appears in a recent comment at or after the latest explicit unblock — a prior worker already opened a PR; older PR handoff URLs are ignored once the task is deliberately unblocked for PR feedback). The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
