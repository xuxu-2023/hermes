...
**Nested routing:** If `delegation.nested_model` is set, nested worker subagents default to that model. Hermes escalates nested children back to `delegation.model` when the nested task looks hard/high-stakes (for example, debugging, risk review, architecture, or trading work), or when the nested child is itself an orchestrator. This keeps routine leaf workers fast while preserving a stronger path for complex nested work.
