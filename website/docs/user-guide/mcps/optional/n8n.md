...
**Source:** [https://github.com/CyberSamuraiX/hermes-n8n-mcp](https://github.com/CyberSamuraiX/hermes-n8n-mcp)
...
or pick it interactively with `hermes mcp`. Uninstall with `hermes mcp uninstall n8n` (the server's config block is removed; any credentials in `~/.hermes/.env` are preserved).
...
**Environment variables prompted at install time:**

| Variable | Prompt | Required | Secret |
|----------|--------|----------|--------|
| `N8N_BASE_URL` | n8n instance URL | yes | no |
| `N8N_API_KEY` | n8n API key (generate under Settings → API) | yes | yes |

Values are written to `~/.hermes/.env`. Non-secret env vars also go to `.env` to keep one credential store.

## Install

**Type:** `git`

**Repository:** `https://github.com/CyberSamuraiX/hermes-n8n-mcp.git` (ref: `main`)
...
The manifest below is the source of truth. It lives at `optional-mcps/n8n/manifest.yaml` in the hermes-agent repo.
