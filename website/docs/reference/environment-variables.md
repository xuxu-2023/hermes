...
| `DISCORD_AUTO_THREAD_CHANNELS` | Comma-separated free-response channel IDs that should still auto-create threads; `DISCORD_NO_THREAD_CHANNELS` remains the hard opt-out. Prefer `discord.auto_thread_channels` in `config.yaml` for normal configuration. |
