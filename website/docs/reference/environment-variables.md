...
| `HERMES_ENVIRONMENT_HINT` | Extra free-text environment hint added to the environment block of the system prompt (e.g. proxy, mounts, sandbox). Takes precedence over the `agent.environment_hint` config key when set. |
