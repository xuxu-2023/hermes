...
| `HERMES_PLUGIN_PATHS` | Additional plugin discovery roots, separated by the platform path separator (`:` on macOS/Linux, `;` on Windows). Each root may be a plugin collection or a direct plugin checkout with a root `plugin.yaml`; discovered plugins still require `plugins.enabled`. |
