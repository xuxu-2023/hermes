...
| `create "<title>"` | Create a new task on the active board. Flags: `--body`, `--assignee`, `--parent` (repeatable), `--workspace scratch\|worktree\|dir:<path>`, `--branch` (branch name for worktree tasks), `--tenant`, `--priority`, `--triage`, `--idempotency-key`, `--max-runtime`, `--max-retries`, `--created-by` (author name, default `user`), `--skill` (repeatable), `--goal` (wrap worker in goal loop — re-evaluates after each turn until acceptance criteria met or budget exhausted; budget-exhausted cards auto-block for human review), `--goal-max-turns N` (turn budget for `--goal`, default 20), `--initial-status {blocked,running}` (override the initial status; useful for pre-claiming a slot before populating the body), `--json`. |
