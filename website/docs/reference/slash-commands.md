...
| `/opportunities [enable\|scan\|seed\|accept\|dismiss N]` (aliases: `/opps`, `/opportunity`) | Review opt-in proactive proposals for learning skills, creating vertical bundles, building specialist profiles, designing Kanban/swarm agent ecosystems, or setting up recurring automations. See [Proactive Opportunities](/user-guide/features/proactive-opportunities). |
...
| `/opportunities [enable\|scan\|seed\|accept\|dismiss N]` | Review opt-in proactive proposals. Accepting an opportunity sends a normal Hermes turn that can learn a skill, create a bundle/profile, design a Kanban/swarm workflow, or set up automation through existing tools. |
...
- `/status`, `/version`, `/background`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/debug`, `/fast`, `/footer`, `/curator`, `/kanban`, `/credits`, `/suggestions`, `/opportunities`, `/blueprint`, `/learn`, `/sessions`, and `/yolo` work in **both** the CLI and the messaging gateway.
