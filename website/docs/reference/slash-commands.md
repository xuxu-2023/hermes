...
| `/proactive [--days N] [--source platform]` (alias: `/opportunities`) | Preview repeated task patterns Hermes could package as a skill, quick command, scheduled job, Kanban template, or workflow runbook. Read-only: it does not create anything without follow-up confirmation. |
...
| `/proactive [--days N] [--source platform]` | Preview Proactive Mode opportunities from recent session history. It reports repeated task shapes with evidence and recommended artifact types, but never creates jobs, skills, commands, memories, or workflows by itself. |
...
- `/status`, `/version`, `/background`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/debug`, `/fast`, `/footer`, `/curator`, `/kanban`, `/credits`, `/suggestions`, `/proactive`, `/blueprint`, `/learn`, `/sessions`, and `/yolo` work in **both** the CLI and the messaging gateway.
