...
| `/handoff <platform>` | **CLI only for live session transfer.** Hand the current session off to a messaging platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix). The gateway picks it up immediately, creates a fresh thread on platforms that support threads (Telegram topics, Discord text-channel threads, Slack message-anchored threads), re-binds the destination to your CLI session_id so the full role-aware transcript replays, and forges a synthetic user turn so the agent confirms it's working in the new place. Your CLI exits cleanly on success with a `/resume` hint; resume locally any time with `/resume <title>`. Refused mid-turn. Requires the gateway to be running and a home channel configured for the target platform (`/sethome` from the destination chat). See [Cross-Platform Handoff](/user-guide/sessions#cross-platform-handoff). |
| `/handoff inline [mission]` | Create a portable markdown handoff document inline in the current chat/session. Works in CLI and messaging gateways. Useful when you want a narrow, artifact-first brief for a fresh session or another platform. |
| `/handoff save [path] [mission]` | Create and save a portable markdown handoff document. In CLI, omitting `path` saves under `~/.hermes/sessions/handoffs/`. In messaging gateways, the saved path is returned in chat. On Slack, use `/hermes handoff save ...` because Slack's native slash-command cap is tight. |
| `/handoff consume <path>` | Load a saved handoff markdown file and queue it as the next agent turn. Works in CLI and messaging gateways; on Slack use `/hermes handoff consume ...`. |
...
- `/skin`, `/snapshot`, `/reload`, `/tools`, `/toolsets`, `/browser`, `/config`, `/cron`, `/platforms`, `/paste`, `/image`, `/statusbar`, `/plugins`, `/busy`, `/indicator`, `/redraw`, `/clear`, `/history`, `/save`, `/copy`, `/billing`, and `/quit` are **CLI-only** commands.
- `/handoff <platform>` is **CLI-only** because it transfers a live CLI session to another platform, but the portable markdown handoff submodes (`/handoff inline`, `/handoff save`, `/handoff consume`) work on CLI and messaging gateways. On Slack, use `/hermes handoff ...` for those submodes because Slack may not expose a dedicated native `/handoff` slash due to the 50-command cap.
